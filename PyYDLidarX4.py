...
from math import atan,pi,floor,cos,sin,radians
...
class YdLidarX4:
    ...
    def __init__(self,port,chunk_size=6000,no_value=0,step_angle=10):
        """Initialize the connection and set port and baudrate.
        step_angle = factor for dividing angles (10: 1 value each 0.1°,2: 1 value each 0.5°)"""
        self._port = port
        ...
        if(self._is_connected):
            if(not self._is_scanning):
                ...
                while self._is_scanning == True:
                    for i in range(0,3600):  #hack: step interval custom
                        distdict.update({i/self._setpangle:[]})
                        ...
                        try:
                            if(e[0]==0):
                                if(YdLidarX4._CheckSum(e)):
                                    ...
                                    for ele in d:
                                        angle = floor(ele[1]*self._setpangle)/self._setpangle
